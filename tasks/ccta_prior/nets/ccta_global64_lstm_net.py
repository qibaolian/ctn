...
class DASERes64LstmNet18(nn.Module):

    def __init__(self, segClasses=2, k=16):

        super(DASERes64LstmNet18, self).__init__()
        ...
        self.inplanes = k
        self.layer2 = self._make_layer(BasicBlock, 2 * k, 6, kernel_size=(3, 3, 3), stride=2)

        self.class2 = Decoder(2 * k, k, k)
        self.class1 = Decoder(k, k, k)
