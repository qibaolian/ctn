...
class GlobalNet2(nn.Module):
    ...
    def __init__(self, segClasses=2, k=16, attention=True, heatmap=False):
        ...
        self.layer3 = self._make_layer(BasicBlock, 4 * k, 2, kernel_size=(3, 3, 3), stride=2)
        # self.layer4 = self._make_layer(BasicBlock, 8 * k, 2, kernel_size=(1, 3, 3), stride=(1, 2, 2))

        if attention:
            self.class4 = DANetHead(4 * k, 4 * k)

        self.class3 = Decoder(4*k + 2*k, 2*k, k)
        self.class2 = Decoder(k + k, 2*k, k)
        self.class1 = Decoder(k + k, 2*k, k)

        self.class0 = nn.Sequential(
            nn.Conv3d(4*k, k, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ELU(inplace=True),
