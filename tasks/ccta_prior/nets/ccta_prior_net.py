...
class DASEResPriorNet18(nn.Module):
    ...
    def __init__(self, segClasses=2, k=16, attention=True, heatmap=False, projection=True):
        ...
        if self.projection:
            self.rnn0_lr = nn.LSTM(segClasses, segClasses, bidirectional=True)
            self.rnn0_ud = nn.LSTM(2*segClasses, segClasses, bidirectional=True)
            self.rnn1_lr = nn.LSTM(segClasses, segClasses, bidirectional=True)
            self.rnn1_ud = nn.LSTM(2 * segClasses, segClasses, bidirectional=True)
            self.rnn2_lr = nn.LSTM(segClasses, segClasses, bidirectional=True)
            self.rnn2_ud = nn.LSTM(2 * segClasses, segClasses, bidirectional=True)
            self.merge_proj = nn.Conv3d(3*2*segClasses, segClasses, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward_seg(self, x):
        ...
        if self.projection:
            seg0 = self.project0(seg)
            seg1 = self.project1(seg)
            seg2 = self.project2(seg)
            seg_proj = torch.cat([seg0, seg1, seg2], 1)
            seg_proj = self.merge_proj(seg_proj)
            seg += seg_proj
    ...
    def project0(self, x):
        ...
        x = x.reshape(w, b * h, c)
        x = self.rnn0_lr(x)[0]
        x = x.reshape(w, b, h, 2 * c)  # WxBxHxC
        x = x.permute(2, 1, 0, 3)  # HxBxWxC
        x = x.reshape(h, b * w, 2 * c)
        x = self.rnn0_ud(x)[0]
        x = x.reshape(h, b, w, 2 * c)  # HxBxWxC
        x = x.permute(1, 3, 0, 2)  # BxCxHxW
        x = x.reshape(b, 2 * c, 1, h, w)  # BxCxDxHxW
    ...
    def project1(self, x):
        ...
        x = x.reshape(w, b * d, c)
        x = self.rnn1_lr(x)[0]
        x = x.reshape(w, b, d, 2 * c)  # WxBxDxC
        x = x.permute(2, 1, 0, 3)  # DxBxWxC
        x = x.reshape(d, b * w, 2 * c)
        x = self.rnn1_ud(x)[0]
        x = x.reshape(d, b, w, 2 * c)  # DxBxWxC
        x = x.permute(1, 3, 0, 2)  # BxCxDxW
        x = x.reshape(b, 2 * c, d, 1, w)  # BxCxDxHxW
    ...
    def project2(self, x):
        ...
        x = x.reshape(h, b * d, c)
        x = self.rnn2_lr(x)[0]
        x = x.reshape(h, b, d, 2 * c)  # WxBxDxC
        x = x.permute(2, 1, 0, 3)  # DxBxHxC
        x = x.reshape(d, b * h, 2 * c)
        x = self.rnn2_ud(x)[0]
        x = x.reshape(d, b, h, 2 * c)  # DxBxHxC
        x = x.permute(1, 3, 0, 2)  # BxCxDxH
        x = x.reshape(b, 2 * c, d, h, 1)  # BxCxDxHxW
