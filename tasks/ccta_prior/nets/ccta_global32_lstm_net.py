...
class DASERes32LstmNet18(nn.Module):

    def __init__(self, segClasses=2, k=16, attention=True, heatmap=False, use_2d_map=False, use_concat=False):

        super(DASERes32LstmNet18, self).__init__()
        ...
        self.layer3 = self._make_layer(BasicBlock, 4 * k, 4, kernel_size=(3, 3, 3), stride=2)

        if attention:
            self.class3 = DANetHead(4 * k, 4 * k)

        self.use_2d_map = use_2d_map
        self.use_concat = use_concat
        if self.use_2d_map:
            self.rnn2 = self._mask_rnn_layer(k)
            ...
            self.rnn3 = self._mask_rnn_layer(2*k)
            self.merge3 = nn.Sequential(
                nn.Conv3d(merge_multiplier*2*k, 2*k, kernel_size=3, stride=1, padding=1, bias=False),
                nn.ReLU(inplace=True)
            )

        self.class2 = Decoder(4 * k + 2 * k, 2 * k, k)
        self.class1 = Decoder(k + k, 2 * k, k)

        self.class0 = nn.Sequential(
            nn.Conv3d(3 * k, 2 * k, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ELU(inplace=True),
    ...
    def forward_seg(self, x):
        ...
        if self.attention:
            d3 = self.class3(self.layer3(e2))
        else:
            d3 = self.layer3(e2)

        if self.use_2d_map:
            e1 = self.project(self.rnn2, self.merge2, e1)
            e2 = self.project(self.rnn3, self.merge3, e2)
