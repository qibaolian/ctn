...
from tasks.intra_vessel_prior.nets.prior_net import DASEResPriorNet18
...
class INTRAVesselPriorSeg(Task):
    
    def __init__(self):
        if cfg.MODEL.STAGE == 'prior':
            super(INTRAVesselPriorSeg, self).__init__()
        else:
            super(INTRAVesselPriorSeg, self).__init_prior__()
    ...
    def get_model(self):
        if cfg.MODEL.NAME == 'da_seresnet18':
            self.net = DASEResPriorNet18(
                ...
                train_prior=cfg.MODEL.STAGE == 'prior'
            )
        else:
    ...
    def train_patches(self, epoch):
        ...
        train_set = VesselSubjectDS(
            {
                ...
                "only_prior": cfg.MODEL.STAGE == 'prior'
            },
            stage="train"
        )
        
        self.net.train()
        meter_names = ['time', 'loss', 'skel_loss', 'hp_loss', 'prior_loss']
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            if cfg.MODEL.STAGE != 'prior':
                code_seg = out['code_seg']
                code_y = out['code_y']
                prior_loss = prior_mse(code_seg, code_y)
                loss += prior_loss
                meters['prior_loss'].update(prior_loss.item(), image.size(0))
            ...
            if batch_idx % cfg.TRAIN.PRINT == 0:
                ...
                if cfg.MODEL.STAGE != 'prior':
                    ss2 += 'prior:%.6f, ' % meters['prior_loss'].avg
                    seg -= meters['prior_loss'].avg
    ...
    def validate_subjects(self, stage='val'):
        ...
        for subject in subjects:
            ...
            para_dict = {"subject": subject, "only_prior": cfg.MODEL.STAGE == 'prior'}
