...
def load_weights_sequential(target, source_state):
    ...
    # print(len(target.state_dict().items()))
    # print(len(source_state.items()))
    # 0.4.1 model's BN layer add "num_batches_tracked", while 0.4.0 pre-train model does not have.
...
class ResNet(nn.Module):
    def __init__(self, block, layers=(3, 4, 23, 3), channel_dim=3):
        self.inplanes = 64
        self.channel_dim = channel_dim
        super(ResNet, self).__init__()
        self.conv1 = nn.Conv2d(self.channel_dim, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
def resnet34(pretrained=True, channel_dim=3):
    model = ResNet(BasicBlock, [3, 4, 6, 3], channel_dim)
    if pretrained:
        load_weights_sequential(model, models.resnet34(pretrained=pretrained).state_dict())
